# Example taken from https://clima.github.io/OceananigansDocumentation/stable/literated/convecting_plankton/
...
N₀(x, z) = 1.0 # nutrient concentration at which growth is zero
growing_and_grazing_P₁(x, z, t, P₁, P₂, N, params) = params.β₁ * N * P₁ / (params.k₁ + N) - params.μ₁ * P₁
growing_and_grazing_P₂(x, z, t, P₁, P₂, N, params) = params.β₂ * N * P₂ / (params.k₂ + N) - params.μ₂ * P₂
growing_and_grazing_N(x, z, t, P₁, P₂, N, params) = params.S₀ * (N - N₀(x, z)) - (1 / params.ρ₁) * params.β₁ * N * P₁ / (params.k₁ + N) - (1 / params.ρ₂) * params.β₂ * N * P₂ / (params.k₂ + N)

# growing_and_grazing(x, z, t, A, B, C, params) = (params.μ₀ * exp(z / params.λ) - params.m) * A
nothing #hide
...
plankton_dynamics_parameters = (μ₁ = 1/day,   # surface growth rate
                                 ...
                                 β₁ = 0.1/day,  # light-dependent growth rate
                                 β₂ = 0.1/day,  # light-dependent growth rate
                                 k₁ = 0.1,      # half-saturation constant for P₁
                                 k₂ = 0.1,      # half-saturation constant for P₂
                                 S₀ = 0.01/day, # nutrient supply rate
                                 ρ₁ = 0.1,      # grazing rate of P₁ by zooplankton
                                 ρ₂ = 0.1)      # grazing rate of P₂ by zooplankton
...
plankton_dynamics_P₁ = Forcing(growing_and_grazing_P₁, field_dependencies = (:P₁, :P₂, :N),
                            parameters = plankton_dynamics_parameters)
plankton_dynamics_P₂ = Forcing(growing_and_grazing_P₂, field_dependencies = (:P₁, :P₂, :N),
                            parameters = plankton_dynamics_parameters)
plankton_dynamics_N = Forcing(growing_and_grazing_N, field_dependencies = (:P₁, :P₂, :N),
                            parameters = plankton_dynamics_parameters)
...
model = NonhydrostaticModel(; grid,
                            ...
                            tracers = (:b, :P₁, :P₂, :N), # P for Plankton
                            buoyancy = BuoyancyTracer(),
                            forcing = (; P₁=plankton_dynamics_P₁, P₂=plankton_dynamics_P₂, N=plankton_dynamics_N),
...
set!(model, b=initial_buoyancy, P₁=1, P₂=1, N=1)
